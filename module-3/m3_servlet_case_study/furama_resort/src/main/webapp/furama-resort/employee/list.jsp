...
<main class="d-flex justify-content-center" style="margin-bottom: 56px;">
    <div id="main-body" class="p-2 col-12 position-relative">
        <a class="btn btn-primary position-absolute" href="/furama-resort/employee?action=create"
           style="left: 28px; top: 24px" role="button">Add New Employee</a>
        <form class="form-inline my-2 my-lg-0 position-absolute" style="right: 28px; top: 24px"
              action="/furama-resort/employee"
              method="get">
            <input class="form-control mr-sm-2" type="search" name="search" placeholder="Employee's name..."
                   aria-label="Search" value="${search}">
            <button class="btn btn-outline-dark my-2 my-sm-0" type="submit">Search</button>
        </form>
        <div id="content" class="bg-light rounded px-2">
            <table class="table text-center">
                <caption style="caption-side: top" class="text-center text-dark"><h2>List of Employees</h2></caption>
                <thead>
                <tr>
                    <th>ID</th>
                    <th>Name</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <c:forEach items="${positions}" var="position">
                                <c:choose>
                                    <c:when test="${position.id == customer.positionId}">${position.name}</c:when>
                                </c:choose>
                            </c:forEach>
                        </td>
                        <td>
                            <c:forEach items="${educationDegrees}" var="educationDegree">
                                <c:choose>
                                    <c:when test="${educationDegree.id == customer.educationDegreeId}">${educationDegree.name}</c:when>
                                </c:choose>
                            </c:forEach>
                        </td>
                        <td>
                            <c:forEach items="${divisions}" var="division">
                                <c:choose>
                                    <c:when test="${division.id == customer.divisionId}">${division.name}</c:when>
                                </c:choose>
                            </c:forEach>
                        </td>
                        <td>${customer.username}</td>
                        <td><a class="btn btn-primary" href="/furama-resort/employee?action=edit&id=${customer.id}"
                               role="button">Edit</a>
                        </td>
                        <td>
        ...
        <nav aria-label="pagination" class="position-relative">
            ...
            <ul class="pagination justify-content-center mt-4">
                <li class="page-item" id="page-first"><a class="page-link"
                                                         href="/furama-resort/employee?page=1${paginationParam}">Fist
                    Page</a>
                </li>
                <li class="page-item" id="page-previous">
                    <a class="page-link"
                       href="/furama-resort/employee?page=${page - 1}${paginationParam}">Previous</a>
                </li>
                <c:forEach items="${pageList}" var="pageItem">
                    <li class="page-item" id="page-${pageItem}"><a class="page-link"
                                                                   href="/furama-resort/employee?page=${pageItem}${paginationParam}">${pageItem}</a>
                    </li>
                </c:forEach>
                <li class="page-item" id="page-next">
                    <a class="page-link" href="/furama-resort/employee?page=${page + 1}${paginationParam}">Next</a>
                </li>
                <li class="page-item" id="page-last"><a class="page-link"
                                                        href="/furama-resort/employee?page=${numberOfPages}${paginationParam}">Last
                    Page</a>
                </li>
            </ul>
            <c:choose>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="/furama-resort/employee" method="get" id="form-btn-delete">
...
<script>
    ...
    $('#staticBackdrop_delete-confirm').on('show.bs.modal', function (event) {
        ...
        const customerInfo = "<p> Do you want to delete employee: <br/>" + button.data('whatever') + " ?</p>";
        const modal = $(this);
